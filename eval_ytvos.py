...
# Model and version
MODEL = 'STM'
print(MODEL, ': Testing on YouTubeVOS')
...
palette = Image.open(DATA_ROOT + '/Annotations/0062f687f1/00000.png').getpalette()
#palette = Image.open(DATA_ROOT + '/Annotations/480p/blackswan/00000.png').getpalette()

offset = 0

def Run_video(info, Fs, Ms, num_frames, num_objects, Mem_every=None, Mem_number=None):
...
def getPreds(pred, Es):
    pred = pred.astype(np.float32)
    objs = np.unique(pred)
    for obj in objs:
        idx = np.where(pred == obj)
        pred[idx] = np.add(pred[idx], Es[int(obj)][idx])
...
for seq, V in enumerate(Testloader):
    ...
    pred, Es = Run_video(info, Fs, Ms, num_frames, num_objects, Mem_every=10000)
    ...
    for f in range(num_frames):
        ...
        img_E.save(os.path.join(test_path, '{:06d}.png'.format(f)))
